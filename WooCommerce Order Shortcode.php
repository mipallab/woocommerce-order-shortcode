...
function set_order_id_cookie( $order_id ) {
    if( ! $order_id ) return;

    // set Cookies
    setcookie( 'custom_order_id', $order_id, time() + 3600, '/' ); // cookies set 1hour
}
add_action( 'woocommerce_thankyou', 'set_order_id_cookie' );

function custom_thank_you_page_shortcode( $atts ) {
   
    // get order id form cookies
